...
function generatePairsHTML(pairs) {
	...
	try {
		var april1 = new Date("2025-03-31T23:00:00-07:00");
		var april2 = new Date("2025-04-01T23:00:00-07:00");

		if(april1.getTime() < DATENOW.getTime() && DATENOW.getTime() < april2.getTime()) {

			var syncPairs = document.getElementById('syncPairs')

			syncPairs.innerHTML = result.replaceAll("Sync Pairs/","Sync Pairs/20250401/");

			var styleAFD = document.createElement("style");
			styleAFD.innerHTML = `
				.syncpairAFD, .pauseAFD { width: 100px; height: 100px; position: relative; margin: 3px; }
				.pauseAFD:hover { cursor: pointer; }
				.syncimagesAFD { width: 100%; transition-duration: 100ms; transform: scale(1) }
				.syncimagesAFD:hover { transform: scale(0.96); }
				.syncimagesAFD:active { transform: scale(0.92); }
				.synclevelAFD { position: absolute; left: 4px; bottom: 4px; z-index: 1; width: 25px; height: 25px; display: flex; }
				@media only screen and (max-width: 1024px) {
					.syncpairAFD, .pauseAFD { width: 24%; max-width: 100px; height: auto; margin: 3px 1px; }
					.synclevelAFD { width: 28%; height: auto; z-index: 2; left: 3%; bottom: 13%; }
			}`;
			document.head.appendChild(styleAFD);

			var filesAFD = ["Chili_0514_5.png","Cilan_0512_5.png","Cress_0516_5.png","Jupiter_0435_5.png","Mars_0432_5.png","Matt_0319_5.png","Saturn_0454_5.png","Shelly_0319_5.png","Tabitha_0323_5.png","Trista_0113_5.png","Tricia_0242_5.png","Trinnia_0440_5.png","Bellis_0029_5.png"]

			filesAFD.forEach(file => {

				var divAFD = document.createElement("div");
				divAFD.classList.add("syncpairAFD");

				var imageAFD = document.createElement("img");
				imageAFD.src = "Sync Pairs/20250401/" + (Math.random() < 0.33 ? file.replace("_5.png", "_EX.png") : file);
				imageAFD.classList.add("syncimagesAFD");

				var levelAFD = document.createElement("img");
				...
				var children = syncPairs.children;
				var randomIndex = Math.floor(Math.random() * (children.length + 1));
				if (randomIndex >= children.length) {
					syncPairs.appendChild(divAFD);
				} else {
					syncPairs.insertBefore(divAFD, children[40 + randomIndex]);
				}
			});

			var divPauseAFD = document.createElement("div");
			divPauseAFD.classList.add("pauseAFD");

			var imagePauseAFD = document.createElement("img");
			imagePauseAFD.src = "Sync Pairs/20250401/0_AFD_Pause1.png";
			imagePauseAFD.classList.add("syncimagesAFD");

			divPauseAFD.appendChild(imagePauseAFD);

			const changeSrcImagePauseAFD = (opt) => {
				const currImg = document.querySelector(".pauseAFD .syncimagesAFD");

				if(!currImg) return;

				const imageMap = {
					"over": {
						"Pause1": "Sync Pairs/20250401/0_AFD_Pause2.png",
						"Pause3": "Sync Pairs/20250401/0_AFD_Pause4.png"
					},
					"out": {
						"Pause1": "Sync Pairs/20250401/0_AFD_Pause1.png",
						"Pause2": "Sync Pairs/20250401/0_AFD_Pause1.png",
						"Pause3": "Sync Pairs/20250401/0_AFD_Pause3.png",
						"Pause4": "Sync Pairs/20250401/0_AFD_Pause3.png"
					},
					"click": {
						"Pause1": "Sync Pairs/20250401/0_AFD_Pause3.png",
						"Pause2": "Sync Pairs/20250401/0_AFD_Pause3.png",
						"Pause3": "Sync Pairs/20250401/0_AFD_Pause1.png",
						"Pause4": "Sync Pairs/20250401/0_AFD_Pause1.png"
					}
				};

				for(const key in imageMap[opt]) {
					if(currImg.src.includes(key)) {
						currImg.src = imageMap[opt][key]; break;
					}
				}
			};

			divPauseAFD.addEventListener("mouseover", () => changeSrcImagePauseAFD("over"));
			divPauseAFD.addEventListener("mouseout", () => changeSrcImagePauseAFD("out"));
			
			divPauseAFD.addEventListener('click', () => {
				document.querySelectorAll('.syncImages img').forEach((img) => {
					if(img.src.includes('/20250401')) {
						img.src = img.src.replace('Sync%20Pairs/20250401', 'Sync%20Pairs');
					} else {
						img.src = img.src.replace('Sync%20Pairs', 'Sync%20Pairs/20250401');
					}
				});
				document.querySelectorAll('.syncpairAFD').forEach((syncpair) => {
					syncpair.classList.toggle("hide")
				});

				changeSrcImagePauseAFD("click");
			});

			document.getElementById("syncPairs").insertBefore(divPauseAFD, syncPairs.children[10 + Math.floor(Math.random() * 30)]);
